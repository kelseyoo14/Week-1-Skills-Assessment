...
def smallest_int(number_list):
    ...
    if len(number_list) == 0:
        ...
        small_int = number_list[0]
        for number in number_list:
            new_num = number
            if new_num < small_int:
                small_int = new_num
        return small_int
...
def largest_int(number_list):
    ...
    if len(number_list) == 0:
        ...
        large_int = number_list[0]
        for number in number_list:
            new_num = number
            if new_num > large_int:
                large_int = new_num
        return large_int
...
def average(number_list):
    ...
    for number in number_list:
        total += number
    number_of_nums = len(number_list)
    mean = total/float(number_of_nums)
...
def reverse_list(my_list):
    ...
    index = len(my_list)
    while index > 0:
        reversed_list.append(my_list[index - 1])
        index -= 1
...
def reverse_list_in_place(my_list):
    ...
    """
    count = 0
    half_list_len = (len(my_list))/2

    while count < half_list_len:
...
def largest_n_items(input_list, n):
    ...
    while total_count < len(input_list):
        count = 0
        while count < (len(input_list) - 1):
            if input_list[count] > input_list[count + 1]:
                input_list[count], input_list[count + 1] = input_list[count + 1], input_list[count]
            count += 1
        total_count += 1
...
# Well, after I wrote all that is above, I realized that I could have just used sorted! Lol.
# So below is the same function (in comments) but written with 'sorted'.....and with many fewer lines.
